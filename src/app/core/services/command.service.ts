...
export class CommandService {
  ...
  setCommands(): void {
    ...
    this.commands$ = this.githubDataService
      .fetchCommands()
      .pipe(
        map(
          (commands: Command[]) => (this.commandsList = commands.filter((command) => !blacklist.includes(command.key)))
        )
      );
  }
  ...
  private argumentsStringify(commandArgument: CommandArgument[] = []): string {
